...
class pbte5 extends StepForm
{
    ...
    public function handle(Request $request)
    {
        ...
        $distri = ""; // 적용분포
        $fmode = "";
        if(array_key_exists('고장모드선택', $ALL['pbte5'])) {
            $fmode = implode(',', array_filter($ALL['pbte5']['고장모드선택']));
        }
        $smode = "";
        if($ALL['pbte1']['모드선택'] == 'C') {
            $smode = $key1_cd;
    ...
    public function form() {
        ...
        $일차분류 = $steps['pbte2']['1차분류'];
        $이차분류 = $steps['pbte3']['2차분류'];
        $삼차분류 = $steps['pbte4']['3차분류'];

        $사차분류 = DB::table('kgtag')->where('key3_cd', $일차분류)->whereIn('key4_cd', $이차분류)->whereIn('key5_cd', $삼차분류)->pluck('key6_nm', 'key6_cd')->toArray();
        $this->checkbox('4차분류')->options($사차분류)->canCheckAll();

        $this->devider();
        
        $모드선택 = $steps['pbtb1']['모드선택'];
        if($모드선택 == "B") { // 고장모드 == 'B'
