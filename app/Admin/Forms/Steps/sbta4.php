...
class sbta4 extends StepForm
{
    ...
    public function handle(Request $request)
    {
        ...
        $key1_cd = $ALL['sbta1']['플랜트'];
        if (is_array($key1_cd)) {
            $key1_cd = rtrim(implode(',',$key1_cd),',');
        }
        
        //$key2_cd = $ALL['sbta2']['위치'];
        //if (is_array($key2_cd)) {
        //    $key2_cd = rtrim(implode(',',$key2_cd),',');
        //}
        $key2_cd = 'ALL';
        
        $key3_cd = $ALL['sbta2']['1차분류'];
        if (is_array($key3_cd)) {
            $key3_cd = rtrim(implode(',',$key3_cd),',');
        }
        
        $key4_cd = $ALL['sbta3']['2차분류'];
        if (is_array($key4_cd)) {
            $key4_cd = rtrim(implode(',',$key4_cd),',');
        }
        
        $key5_cd = "";
        if(array_key_exists('3차분류', $ALL['sbta3'])) {
            $key5_cd = $ALL['sbta3']['3차분류'];
            if (is_array($key5_cd)) {
...
*/
        ...
        $distri = ""; // 적용분포
        $fmode = "";
        //if(array_key_exists('고장모드선택', $ALL['sbta3'])) {
        //    $fmode = implode(',', array_filter($ALL['sbta3']['고장모드선택']));
        //}
        $smode = "";
        //if($ALL['sbta1']['모드선택'] == 'C') {
        //    $smode = $key1_cd;
