...
class analsbteController extends Controller
{
    public function index(Content $content)
    {
        ...
        $form2 = new Widgets\Form();

        $form2->method('get');

        $form2->html('<a href="/anal_sbt_b">기본분석</a>');

        $form2->radio('분석타입선택')->options(['B' => '기본분석', 'E'=> '심화분석'])->default('E')->disable();
        // $form2->radio('분석플레그')->options(['S' => '분석시작', 'W' => '심화분석중', 'F' => '분석종료'])->default('S');
        $form2->dateRange('sdate', 'edate', '기간선택')->help('비워둘경우 오늘날짜입력됨, 도움말참조'); //검토필요
        $form2->divider();      
        $form2->text('관심시간입력')->placeholder('1000,5000,10000,50000,100000')->help('최대10개값입력가능, 도움말참조');
        $form2->radio('적용분포')->options(['1' => '와이블', '2'=> '로그', '3'=>'지수', '4'=>'정규'])->default('3');
        $form2->divider();
        $form2->radio('고장모드선택')->options(['1' => '여', '2' => '부'])->default('2');
        $form2->radio('검정모드선택')->options(['1' => '여', '2' => '부'])->default('2');
        $form2->divider();
        ...
        $form2->multipleSelect('위치')->options($위치)->default('2200')->rules('required');

        $일차분류 = DB::table('kgpbt')->pluck('key3_cd')->toArray();
        $일차분류 = array_combine($일차분류, $일차분류);
        $form2->multipleSelect('1차분류')->options($일차분류)->default('2200')->rules('required');

        $이차분류 = DB::table('kgpbt')->pluck('key4_cd')->toArray();
        $이차분류 = array_combine($이차분류, $이차분류);
        $form2->multipleSelect('2차분류')->options($이차분류)->default('2200')->rules('required');

        $삼차분류 = DB::table('kgpbt')->pluck('key5_cd')->toArray();
        $삼차분류 = array_combine($삼차분류, $삼차분류);
        $form2->multipleSelect('3차분류')->options($삼차분류)->default('2200')->rules('required');

        $사차분류 = DB::table('kgpbt')->pluck('key6_cd')->toArray();
        $사차분류 = array_combine($사차분류, $사차분류);
        $form2->multipleSelect('4차분류')->options($사차분류)->default('2200')/* ->rules('required') */;

        $삼다시일차분류 = DB::table('kgpbt')->pluck('key3_1_cd')->toArray();
        $삼다시일차분류 = array_combine($삼다시일차분류, $삼다시일차분류);
        $form2->multipleSelect('1차추가분류')->options($삼다시일차분류)->default('2200')->rules('required');
