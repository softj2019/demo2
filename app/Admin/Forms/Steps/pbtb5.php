...
class pbtb5 extends StepForm
{
    ...
    public function handle(Request $request)
    {
        ...
        $distri = ""; // 적용분포
        $fmode = "";
        if(array_key_exists('고장모드선택', $ALL['pbtb5'])) {
            $fmode = implode(',', array_filter($ALL['pbtb5']['고장모드선택']));
        }
        $smode = "";
        if($ALL['pbtb1']['모드선택'] == 'C') {
            $smode = $key1_cd;
    ...
    public function form() {
        ...
        $일차분류 = $steps['pbtb2']['1차분류'];
        $이차분류 = $steps['pbtb3']['2차분류'];
        $삼차분류 = $steps['pbtb4']['3차분류'];
