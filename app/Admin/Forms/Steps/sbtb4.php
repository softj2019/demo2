...
class sbtb4 extends StepForm
{
    ...
    public function handle(Request $request)
    {
        ...
        $key1_cd = $ALL['sbtb1']['플랜트'];
        if (is_array($key1_cd)) {
            $key1_cd = rtrim(implode(',',$key1_cd),',');
        }
        
        //$key2_cd = $ALL['sbtb2']['위치'];
        //if (is_array($key2_cd)) {
        //    $key2_cd = rtrim(implode(',',$key2_cd),',');
        //}
        $key2_cd = 'ALL';

        $key3_cd = $ALL['sbtb2']['1차분류'];
        if (is_array($key3_cd)) {
            $key3_cd = rtrim(implode(',',$key3_cd),',');
        }
        
        $key4_cd = $ALL['sbtb3']['2차분류'];
        if (is_array($key4_cd)) {
            $key4_cd = rtrim(implode(',',$key4_cd),',');
        }

        $key5_cd = "";
        if(array_key_exists('3차분류', $ALL['sbtb3'])) { 
            $key5_cd = $ALL['sbtb3']['3차분류'];
            if (is_array($key5_cd)) {
...
*/
        ...
        $distri = ""; // 적용분포
        $fmode = "";
        if(array_key_exists('고장모드선택', $ALL['sbtb3'])) {
            $fmode = implode(',', array_filter($ALL['sbtb3']['고장모드선택']));
        }
        $smode = "";
        if($ALL['sbtb1']['모드선택'] == 'C') {
            $smode = $key1_cd;
